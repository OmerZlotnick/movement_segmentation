...
LocationRawData <- function(Start_Time_Str,End_Time_Str,ListOfStart,TagPrefix=972006000000,quaryVar='TAG,TIME,X,Y,VARX,VARY,COVXY,NBS') {
...
tags <- unique(ListOfStart$Tag.ID)
dir.create("../outputs", showWarnings = FALSE)
dir.create("../outputs/1 locations_data", showWarnings = FALSE)
i <- 1

for(tag in tags){
  tag_df <- data.frame(TAG = tag)
  start_date <- ListOfStart[ListOfStart$Tag.ID == tag,]$Date.capture
  start_hour <- ListOfStart[ListOfStart$Tag.ID == tag,]$Start.hour
  
  start_date <- format(dmy(start_date), format="%Y-%m-%d")
  start <- paste(start_date, start_hour)
  
  end_date <- ListOfStart[ListOfStart$Tag.ID == tag,]$status_date
  end_date <- format(dmy(end_date), format="%Y-%m-%d")
  end <- paste(end_date, "01:00:00")
  
  
  tag_locations_df <- LocationRawData(start,end,tag_df)
  #tag_locations_df <- LocationRawData(start,'2022-05-02 07:00:00',tag_df) # Download the data I want
  output_path <- paste("../outputs/1 locations_data/", as.character(as.numeric(tag)), ".csv", sep = "")
  write.csv(tag_locations_df, output_path, row.names = FALSE)
  
  print(tag)
  print(paste(as.character(i), "out of", as.character(length(tags))))
  i <- i + 1
}
